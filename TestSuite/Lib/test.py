...
import os
import robot
from AppiumLibrary.keywords import _ScreenshotKeywords


class _Screenshot_Extended(_ScreenshotKeywords ):

    def __init__(self):
        _ScreenshotKeywords._screenshot_index = 0
    # Public

    def Get_page_screenshot(self, filename=None):
    ...
    def _capture_screenshot_paths(self, filename):
        if not filename:
            self._capture_index += 1
            filename = 'appium-screenshot-%d.png' % self._capture_index
        else:
            self._screenshot_index += 1
            filename = filename + '-%d.png' % self._capture_index
            # filename = filename.replace('/', os.sep)
        logdir = self._get_log_dir()
        path = os.path.join(logdir, filename)
        link = robot.utils.get_link_path(path, logdir)
