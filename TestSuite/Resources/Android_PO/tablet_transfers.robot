...
Make Transfer Beetween Account
    ...
    ${dicloc}      AppiumLibrary.get element location    id=carousel_accounts_from
    ${c1}    AppiumLibrary.get element size   id=carousel_accounts_from
...
Make Transfer Beetween Account Future
    ...
    log to console  ${account1}
    go to transfer between my accounts
    Select Between My Accounts For Tablets   ${account1}  ${account2}
    AppiumLibrary.input text  id=et_amount    1.00
    run keyword and ignore error     AppiumLibrary.hide keyboard
    Scroll Element Down To View   id=bt_continue
    get screenshot
    AppiumLibrary.click element   id=et_date
    set transfers calendar date  20170601
    AppiumLibrary.click element  id=bt_continue
    AppiumLibrary.wait until element is visible   xpath=//*[contains(@text,'TRANSFER DATE')]
    AppiumLibrary.click element  id=bt_continue
#    AppiumLibrary.wait until element is visible   id=tv_transaction_completed
...
Make Transfer To Someone else
    ...
    select account from top slider  *8526
    AppiumLibrary.input text  id=et_amount    1.00
    run keyword and ignore error     AppiumLibrary.hide keyboard
    AppiumLibrary.swipe  1000  614  0  100  2000
    get screenshot
    AppiumLibrary.click element  id=bt_continue
    AppiumLibrary.wait until element is visible   xpath=//*[contains(@text,'TRANSFER DATE')]
    AppiumLibrary.click element  id=bt_continue
...
Make Transfer To Someone Else Future
    ...
    AppiumLibrary.wait until element is visible   id=et_amount
    #scroll  xpath=("//*[contains(@resource-id,'id/carousel_accounts_from')]/android.widget.TextView[@index='1']")  xpath=//android.support.v4.view.ViewPager[1]/android.widget.TextView[2]
    #Drag And Drop By Offset  xpath=//*[contains(@resource-id,'id/carousel_accounts_from')]
    AppiumLibrary.swipe  1000  614  100  614  2000
    #AppiumLibrary.swipe  1000  614  0  614  2000
    AppiumLibrary.input text  id=et_amount    1.00
